...
## Clean data
gs_v2 <- clean_matches_data(atp_matches = atp_matches,
...
gs_v2 <- extract_sgp(gs_v2)

usethis::use_data(gs_v2, overwrite = TRUE)

## Make into player data
gs_players_v2 <- matches_to_player_data(gs_v2,
...
gs_pbp_v2 <- as.data.frame(summarize_pbp(gs_point_by_point,
                        start_year = 2013,
                        end_year = 2020))

usethis::use_data(gs_pbp_v2, overwrite = TRUE)

data(gs_v2)

## Join gs_pbp and gs together

gs_join <- join_gs_pbp(gs_pbp_v2, gs_v2)

gs_partial_v2 <- as.data.frame(combine_fields(gs_join))


head(gs_partial[, c("winner_name", "loser_name", "player1", "player2", "w_n_sv")])

usethis::use_data(gs_partial_v2, overwrite = TRUE)

## gs_partial_players
devtools::load_all("../") ## Load court sports functions
data(gs_partial_v2)
gs_partial_players_v2 <- matches_to_player_data(gs_partial_v2)

usethis::use_data(gs_partial_players_v2, overwrite = TRUE)
