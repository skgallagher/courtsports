...
ggplot( aes(log(ht+1), log(ace+1), color = factor(did_win))) + 
  geom_jitter() + 
  scale_color_manual(guide=FALSE, values = win_colors, labels = c("No", "Yes")) + 
  geom_smooth(method = "lm") + 
  geom_label_repel(data=subset(filter(gs_players, league == "ATP"),
                               (log(ht+1) > 5.25 & log(ace+1) > 3.9) |
...
ggplot( aes(log(ht+1), log(ace+1), color = factor(did_win))) + 
  geom_jitter() + 
  scale_color_manual("Winner?", values = win_colors, labels = c("No", "Yes")) + 
  geom_smooth(method = "lm") + 
  geom_label_repel(data=subset(filter(gs_players, league == "WTA"),
                               (log(ht+1) > 5.22 & log(ace+1) > 2.6) |
                                (log(ht+1) > 5.215 & log(ace+1) < .5) | 
...
## Mixed-effects models for all grand slams
...
knitr::kable(model.sums, format = "latex",
             row.names = FALSE, col.names = c("Model", "AIC", "EDF"),
...
If we examine the fixed effects for all seven models, and the random effects for the `ind_logistic` and `ind_logistic_noioc`, we see that the model structure chosen does not substantially change the parameter estimates. Across all models, `rank` and `opponent_rank` appear to best explain the probability of winning the match. Other fixed effects in the models, like whether it was a late round in the grand slam, the year, or ATP/WTF, were not found to be significantly different than zero. 
...
knitr::kable(round(cor(ranef(ind_logistic_noioc)$name_int),2), format = "latex",
             row.names = TRUE, col.names = c("Australian Open", "French Open", "US Open", "Wimbledon"),
...
Rank and opponent's rank clearly have the largest effect on predicting the winner. Looking at the individual effects for Serena, Nadal, and Federer; they actually fall near the average for all four grand slams. We now look at a similar model, but instead of using `win' as the outcome variable, we'll look at game statistics that may be more sensitive to court surface -- aces, winners, net points won, and unforced errors -- and see if individual differences are detectable. 

(some plots that show a) Serena/Federer are stronger servers, b) Federer makes fewer unforced errors at Wimbledon, c) something about Nadal)
