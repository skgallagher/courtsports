...
wim_inds <- which(gs$tournament == "Wimbledon")
head(gs[wim_inds,])

serena_inds <- which(gs_partial_players$name == "Serena Williams")
gs_partial_players[serena_inds[1],]

head(gs_partial)
djoker_inds <- which(gs_partial_players$name == "Novak Djokovic")
gs_partial_players[djoker_inds[1],]
...
The four tennis grand slams (Australian Open (AO), French Open (FO),  Wimbledon (Wim.), US Open (USO)) are played in different cities (Melbourne, Paris, London, New York City) at different times of the year (January, May, June, August) over the course of two weeks with 7 total rounds and 128 players for both men (ATP) and women (WTA).  All four slams are played on different surfaces (Plexicushion hard court, clay, grass, DecoTurf hard court).  It is often thought that top players perform better on certain surfaces. Recently, Spanish players, led by Nadal, seem to have dominated the clay court scene [@tp2018]. In addition, there is the idea of "home court advantage" in which players from the country of the grand slam tournament are thought to perform better at home than at other grand slams.  
...
Few academic papers have been written about Nadal, Federer, or Williams.  @leitner2009 study Federer's odds of winning when Nadal suddenly withdrew from Wimbledon and shows that Federer was too heavily favored by bookmakers .  @five2015 shows how Williams has gotten better with age, even past the point when other greats began to decline, but the study does not look at surface type.
...
The rest of this paper is organized as follows. In Section 2 we describe the grand slam tennis data and examine it at a high level.  In Section 3 we describe our models with (1) complete data with few features and (2) partial data with more features along with the results.  Finally in Section 4, we summarize our results and discuss future work. 
...
ggplot(df, aes(x = coeff, y = ave, col = player)) +
  geom_errorbar(aes(ymin = ave - 2 *sd, ymax = ave + 2 * sd), position = "dodge", width = .3) + 
   my_theme +
  # coord_flip() +
  theme(axis.text.x=element_text(angle = 30, hjust = 1)) +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = .4) + 
  ggtitle("Effects on Reference Court") +
  scale_color_brewer("",palette = "Dark2") +
  theme(legend.position = "bottom", legend.text=element_text(size=8),
        # legend.margin=margin(0, 0, 0, 0),
        legend.margin=margin(t = 0, unit='cm'),
  ...
  xlab("") + coord_flip() + 
  ylab("Estimate") +
  scale_x_discrete(labels = c("Intercept", "Opp. Rank", "% Aces", "% Break Points Won", "% Net Points Won", "Ave. Serve Speed", "Winner:UE"))
  guides(col = guide_legend(nrow=2))
  #ggsave("~/Documents/ref-courts-eff.pdf", width = 8, height = 6)
```
...
keep_coefs <- which(df$coeff %in% c("(Intercept)", "opponent_rank",
  ...
  ## Interaction Effects
df2 <- df[-keep_coefs,]
ggplot(df2, aes(x = coeff, y = ave, col = player)) +
  geom_errorbar(aes(ymin = ave - 2 *sd, ymax = ave + 2 * sd), position = "dodge", width = .3) + 
   my_theme +
  # coord_flip() +
  theme(axis.text.x=element_text(angle = 30, hjust = 1)) +
  ...
  theme(legend.position = "bottom", legend.text=element_text(size=8),
        ...
        legend.box.margin=margin(-10,-10,-10,-10)) + 
  xlab("") +
  ylab("Estimate") + facet_grid(~player) + coord_flip() + 
  #scale_x_discrete(labels = c("Intercept", "Opp. Rank", "% Aces", "% Break Points Won", "% Net Points Won", "Ave. Serve Speed", "Winner:UE"))
  guides(col = guide_legend(nrow=2))
```
...
Individual linear models show that Nadal is best on clay, Federer is best on grass, and Williams is strong in all grand slams, which also agrees with prior conceptions.  These models also show that features such as aces and break points are important.

We often found that our analyses are restricted by available data. Due to the large amount of grand slam data available, we are able to fit individual models for Federer, Nadal, and Williams that provided substantially more information about their performance on different court surfaces than the more general models which included all players. Point by point data from matches among lower-profile players is more likely to be missing or contain errors.  Ideally, these types of models would be possible for all players, not just those with many grand slam appearances. 

Looking forward, we would like to better explore the point by point data, especially for the attributes which were excluded in this analysis: rally length, minutes played, and distance covered.  We would also like to include data from Masters level tournaments, which would give us a clearer view if the differences we found in this paper are due to court surface or the slam itself.  Another way we could adjust for grand slam confounders by incorporating weather conditions and matches played in the last few months.  Additionally, we would like to be able to adjust for performance over time as new competitors arrive and older players decline.
