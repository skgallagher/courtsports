...
If we examine the fixed effects for all seven models, we see that the model structure chosen does not substantially change the parameter estimates. Across all models, `rank` and `opponent_rank` appear to best explain the probability of winning the match. Other fixed effects in the models, like whether it was a late round in the grand slam, the year, or ATP/WTF, were not found to be significantly different than zero. If we look at the random effects for Williams, Federer, and Nadal, however, the individual effects are not very informative (especially after taking uncertainty into consideration). 
...
### Linear regression for other stats

Rank and opponent's rank clearly have the largest effect on predicting the winner. Looking at the individual effects for Serena, Nadal, and Federer; they actually fall near the average for all four grand slams. We now look at a similar model, but instead of using `win' as the outcome variable, we'll look at game statistics that may be more sensitive to court surface -- aces, net points won, and unforced errors -- and see if individual differences are detectable. We fit three additional models, each of the form $Y_i = \beta X_i + \beta_{T[i]}T$ with $\beta_{T[i]} = \alpha_i T_i$, with $X_i$ including the same variables as the model above. 
...
If we look at the random effects for Williams, Federer, and Nadal (FIGURE REF), differences are much more noticeable than in the logistic regression model. For instance, we would predict more aces for Williams at the Australian Open and Wimbledon than at the French Open. We also see that Federer's performance at Wimbledon is consistently better than other tournaments in both aces and unforced errors. Interestingly, an increased performance on clay is not observed for Nadal in aces, net winners, or unforced errors. 
...
For Federer, the model with the lowest AIC has negative coefficients for AO, FO, and USO. vs Wim., and W/UE,  It has positive coefficients for opponent rank, percent of break points won, and all the interaction terms: court and W/UE.

The standardized residuals plot shown in Figure \ref{fig:individ-resids}  shows that the model is a fairly good fit for predicting the percent of points won, but almost seem to see two clusters of residuals splitting with predicted point percentage of about 0.63.  For the significant variables ($\alpha$ = 95\%), while adjusting for the other variables, we expect percent of points won to be $`r signif(abs(coefs_f[2] + coefs_f[8]), 2)` \pm `r signif(2 * sqrt(cov_f[2,2] + cov_f[8,8] + 2 * cov_f[2,8]), 2)`$ less at AO compared to the Wim.; $`r signif(abs(coefs_f[3] + coefs_f[9]), 2)`\pm `r signif(2 * sqrt(cov_f[3,3] + cov_f[9,9] + 2 * cov_f[3,9]), 2)`$ less at FO compared to the Wim.; $`r signif(abs(coefs_f[4] + coefs_f[9]), 2)`\pm `r signif(2 * sqrt(cov_f[4,4] + cov_f[9,9] + 2 * cov_f[4,9]), 2)`$ less at USO compared to the Wim.;  to decrease by $`r signif(abs(coefs_f[6] + coefs_f[8]) , 2)` \pm `r signif(2 * sqrt(cov_f[6,6] + cov_f[8,8] + 2 * cov_f[6,8]), 2)`$ for a one unit increase in W/UE at AO, $`r signif(abs(coefs_f[6] + coefs_f[9]) , 2)` \pm `r signif(2 * sqrt(cov_f[6,6] + cov_f[9,9] + 2 * cov_f[6,9]), 2)`$ for a one unit increase in W/UE at FO, and $`r signif(abs(coefs_f[6] + coefs_f[10]) , 2)` \pm `r signif(2 * sqrt(cov_f[6,6] + cov_f[10,10] + 2 * cov_f[6,10]), 2)`$ for a one unit increase in W/UE at USO.  This is strong evidence that Federer performs best at Wimbledon but possibly does better at the USO, depending on his W/UE ratio.
