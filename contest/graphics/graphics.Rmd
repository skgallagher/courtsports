...
```{r}
filter(gs_players, round == "R128", ioc == "ESP") %>% 
  group_by(tournament) %>% 
  summarise(avg_ranking = mean(rank))

spain_ranks <- filter(gs_players, ioc == "ESP") %>% 
  group_by(tournament, round) %>% 
  summarise(avg_ranking = mean(rank))

ggplot(spain_ranks) + 
  geom_line(aes(round, avg_ranking, color=tournament, group=tournament)) + 
...
## Spain proportion of wins across the tournaments

```{r}
spain_wins <- filter(gs_players, ioc == "ESP") %>% 
  group_by(tournament, round) %>% 
  summarise(win_prop = mean(did_win),
            win_prop_se = sqrt(win_prop*(1-win_prop)/n()))

ggplot(spain_wins, aes(round, win_prop, color=tournament, group=tournament)) + 
  geom_line(cex=2) + 
  geom_point() +
  labs(title = "Proportion of wins among Spaniards") +
  geom_errorbar(aes(ymin = win_prop - 2*win_prop_se,
                    ymax = win_prop + 2*win_prop_se),
                width=.2,
